...
class BaseSensorNetwork(drs.Module):
    ...
    def __init__(
        ...
        # Mirrored states
        self.belief_current_parcel_mass = drs.Variable("belief_parcel_mass", 0.0)
        self.belief_ore_stock = drs.Level(
            "BeliefOreStock_Level", initial_value=self.config.target_ore_stock_level
        )
    ...
    def belief_routing_fraction(self) -> float:
        raise NotImplementedError(
            "Subclasses must define how to calculate the routing fraction."
        )
    ...
    def forward(self):
        ...
        if self.belief_ore1_stock is not None:
            ...
            self.belief_ore1_stock.lower_threshold = (
                self.plant.true_ore1_stock.mass.lower_threshold
            )
        if self.belief_ore2_stock is not None:
            self.belief_ore2_stock.rate = self.plant.true_ore2_stock.mass.rate
            self.belief_ore2_stock.lower_threshold = (
                self.plant.true_ore2_stock.mass.lower_threshold
            )

        self.belief_ore_stock.lower_threshold = (
            self.plant.true_ore_stock.lower_threshold
        )
...
class ConcentratorSensorNetwork(BaseSensorNetwork):
    def __init__(self, config: ConcentratorConfig, mine, fleet, plant):
        ...
        self.belief_ore1_stock = drs.Level(
            "BeliefOre1Stock_Level",
            initial_value=(1 - initial_fraction) * self.config.target_ore_stock_level,
        )
        self.belief_ore2_stock = drs.Level(
            "BeliefOre2Stock_Level",
            initial_value=initial_fraction * self.config.target_ore_stock_level,
        )

        self.belief_current_parcel_grade = drs.Variable("belief_parcel_grade", 0.0)
        self.belief_ore1_grade = drs.Variable("belief_ore1_grade", 0.0)
        self.belief_ore2_grade = drs.Variable("belief_ore2_grade", 0.0)

    @property
    def belief_routing_fraction(self) -> float:
        return (
            self.belief_current_parcel_grade.value / self.config.grade_percentage_scale
        )

    def forward(self):
        super().forward()
        self.belief_current_parcel_grade.value = (
            self.mine.true_current_parcel_grade.value
        )

        true_ore1_grade = self.plant.true_ore1_stock.current_concentration("grade")
        self.belief_ore1_grade.value = max(
            0.0, true_ore1_grade + float(np.random.normal(0, 0.1))
        )

        true_ore2_grade = self.plant.true_ore2_stock.current_concentration("grade")
        self.belief_ore2_grade.value = max(
            0.0, true_ore2_grade + float(np.random.normal(0, 0.1))
        )
...
class CyanidationSensorNetwork(BaseSensorNetwork):
    def __init__(self, config: CyanidationConfig, mine, fleet, plant):
        ...
        self.belief_ore1_stock = drs.Level(
            "BeliefOre1Stock_Level",
            initial_value=(1 - initial_ore2_fraction)
        ...
        self.belief_current_parcel_cyanide = drs.Variable("belief_parcel_cyanide", 0.0)
        self.belief_ore1_cyanide = drs.Variable("belief_ore1_cyanide", 0.0)
        self.belief_ore2_cyanide = drs.Variable("belief_ore2_cyanide", 0.0)
    ...
    def forward(self):
        super().forward()
        self.belief_current_parcel_cyanide.value = (
            self.mine.true_current_parcel_cyanide.value
        )

        true_ore1_cyanide = self.plant.true_ore1_stock.current_concentration("cyanide")
        self.belief_ore1_cyanide.value = max(
            0.0, true_ore1_cyanide + random.gauss(0, 0.1)
        )

        true_ore2_cyanide = self.plant.true_ore2_stock.current_concentration("cyanide")
        self.belief_ore2_cyanide.value = max(
            0.0, true_ore2_cyanide + random.gauss(0, 0.1)
        )
