...
class Node(Module):
    ...
    def __init__(self, name: str, attributes: list, initial_values: dict = None):
        ...
        self.accumulations = {
            attr: Level(
                f"{name}_{attr}_Level", initial_value=initial_values.get(attr, 0.0)
            )
    ...
    def __rshift__(self, other):
        """
        Syntax sugar to create edges easily.
        Usage: node_a >> node_b
        """
        from drs.network import Edge

        if not hasattr(self, "_network_ref"):
            raise ValueError("Nodes must be added to a Network before wiring.")

        edge_name = f"{self.name}_to_{other.name}"
        # Use accumulations.keys() as the canonical attribute list, since subclasses
        # TODO: update stockpile so it doesnt do this and then make this simpler?
        # (e.g. Stockpile) may override self.attributes for backwards compatibility.
        self_attrs = list(self.accumulations.keys())
        other_attrs = list(other.accumulations.keys())
        combined_attrs = list(set(self_attrs + other_attrs))

        new_edge = Edge(edge_name, source=self, target=other, attributes=combined_attrs)
        self._network_ref.add_edge(new_edge)

        # Return 'other' to allow chaining: node_a >> node_b >> node_c
        return other
...
class Network(Module):
    ...
    def add_node(self, node: Node):
        ...
        # Store directly in __dict__ to avoid Module.__setattr__ registering
        # the Network as a sub-module of the Node (which causes infinite recursion
        # in _get_vars due to the circular reference).
        node.__dict__["_network_ref"] = self
    ...
    def compile(self):
        ...
        """
        in_degree = {
            node: len([e for e in node.in_edges if e.source is not None])
